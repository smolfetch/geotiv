// geotiv/writer.hpp
#pragma once

#include <cstdint>
#include <filesystem>
#include <fstream>
#include <stdexcept>
#include <string>
#include <vector>

#include "concord/types_basic.hpp" // for concord::CRS, Datum, Euler
#include "geotiv/types.hpp"        // for geotiv::RasterCollection, Layer

namespace geotiv {
    namespace fs = std::filesystem;

    /// Assemble a little-endian, single-IFD, single-strip GeoTIFF
    /// from the first layer of rc and the provided chunky pixelData.
    inline std::vector<uint8_t> toTiffBytes(RasterCollection const &rc, std::vector<uint8_t> const &pixelData) {
        if (rc.layers.empty())
            throw std::runtime_error("toTiffBytes(): no layers in RasterCollection");

        // Take layer #0
        auto const &L = rc.layers[0];

        // Dimensions & layout
        uint32_t width = L.width;
        uint32_t height = L.height;
        uint32_t spp = L.samplesPerPixel;
        uint32_t pcfg = L.planarConfig;

        // Raw pixel buffer size
        uint32_t pdSz = static_cast<uint32_t>(pixelData.size());

        // The IFD starts immediately after the 8-byte header + pixel data
        uint32_t ifdOffset = 8 + pdSz;

        // Build the ImageDescription: "CRS <WGS|ENU> DATUM lat lon alt HEADING yaw"
        std::string desc = "CRS ";
        desc += (rc.crs == concord::CRS::WGS ? "WGS" : "ENU");
        desc += " DATUM " + std::to_string(rc.datum.lat) + " " + std::to_string(rc.datum.lon) + " " +
                std::to_string(rc.datum.alt) + " HEADING " + std::to_string(rc.heading.yaw);
        uint32_t descLen = static_cast<uint32_t>(desc.size() + 1); // include NUL

        // We'll write exactly 7 tags in our IFD:
        //   ImageWidth, ImageLength, SamplesPerPixel, PlanarConfiguration,
        //   StripOffsets, StripByteCounts, ImageDescription
        uint16_t entryCount = 7;

        // Calculate where the description text will go
        uint32_t descOffset = ifdOffset + 2     // entryCount (2 bytes)
                              + entryCount * 12 // each entry is 12 bytes
                              + 4;              // next-IFD pointer

        std::vector<uint8_t> buf;
        buf.reserve(descOffset + descLen);

        // Helpers to append little-endian integers
        auto appendLE16 = [&](uint16_t v) {
            buf.push_back(uint8_t(v & 0xFF));
            buf.push_back(uint8_t((v >> 8) & 0xFF));
        };
        auto appendLE32 = [&](uint32_t v) {
            buf.push_back(uint8_t(v & 0xFF));
            buf.push_back(uint8_t((v >> 8) & 0xFF));
            buf.push_back(uint8_t((v >> 16) & 0xFF));
            buf.push_back(uint8_t((v >> 24) & 0xFF));
        };

        // --- 1) TIFF header ---
        buf.push_back('I');
        buf.push_back('I');    // little-endian marker
        appendLE16(42);        // TIFF magic number
        appendLE32(ifdOffset); // offset to IFD

        // --- 2) Pixel data strip ---
        buf.insert(buf.end(), pixelData.begin(), pixelData.end());

        // --- 3) IFD entries ---
        appendLE16(entryCount);

        // Tag 256: ImageWidth (LONG = type 4)
        appendLE16(256);
        appendLE16(4);
        appendLE32(1);
        appendLE32(width);

        // Tag 257: ImageLength (LONG)
        appendLE16(257);
        appendLE16(4);
        appendLE32(1);
        appendLE32(height);

        // Tag 277: SamplesPerPixel (SHORT = type 3)
        appendLE16(277);
        appendLE16(3);
        appendLE32(1);
        appendLE32(spp);

        // Tag 284: PlanarConfiguration (SHORT)
        appendLE16(284);
        appendLE16(3);
        appendLE32(1);
        appendLE32(pcfg);

        // Tag 273: StripOffsets (LONG)
        //   we wrote the strip starting at byte 8
        appendLE16(273);
        appendLE16(4);
        appendLE32(1);
        appendLE32(8);

        // Tag 279: StripByteCounts (LONG)
        appendLE16(279);
        appendLE16(4);
        appendLE32(1);
        appendLE32(pdSz);

        // Tag 270: ImageDescription (ASCII = type 2)
        appendLE16(270);
        appendLE16(2);
        appendLE32(descLen);
        appendLE32(descOffset);

        // Next IFD pointer = 0 (no more IFDs)
        appendLE32(0);

        // --- 4) Description text ---
        buf.insert(buf.end(), desc.begin(), desc.end());
        buf.push_back('\0');

        return buf;
    }

    /// Write the GeoTIFF bytes to disk at outPath
    inline void WriteRasterCollection(RasterCollection const &rc, std::vector<uint8_t> const &pixelData,
                                      fs::path const &outPath) {
        auto bytes = toTiffBytes(rc, pixelData);
        std::ofstream ofs(outPath, std::ios::binary);
        if (!ofs)
            throw std::runtime_error("WriteRasterCollection(): cannot open \"" + outPath.string() + "\"");
        ofs.write(reinterpret_cast<char *>(bytes.data()), bytes.size());
    }

} // namespace geotiv
